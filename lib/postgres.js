...
module.exports = {
  ...
  extract: (options) => {
    ...
    return co(function *(){
      ...
      //sequences:
      q = "select relname from pg_class where relkind ='S'";
      var sequencesObject = {};
      (yield conn.query(q)).rows.forEach(r => sequencesObject[r.relname] = r.relname);
      ...
      for (var c of rows) {
        ...
        if (c.column_default != null) {
          var matches = c.column_default.match(/nextval\(\'\"([^"]+)"/);
          if (Array.isArray(matches) && matches.length > 0 && matches[1] in sequencesObject)
      ...
      for (var r of rows) {
        ...
        if (index.type === 'PRIMARY KEY') {
          ...
          schema.tables[r.table_name].columns[r.column_name].isPK = true;
        }
